# Figma-to-the design system Implementation Guide MCP Server
...
MCP server instantly shows:
  - "Use the design system Hero component"
  - "Use the design system Button (primary variant)"
  - "Use the design system Navbar"
    ↓
...
└────────┬────────┘
         │ MCP Protocol
┌────────▼───────────────────────────┐
│  Figma-the design system MCP Server          │
│                                     │
│  ┌───────────────────────────────┐ │
│  │  1. the design system Scanner           │ │
...
│  │     Matches to the design system comps  │ │
│  └───────────────────────────────┘ │
│  ┌───────────────────────────────┐ │
...
```
components-figma-mcp-server/
├── src/
│   ├── index.ts                        # MCP server entry
│   ├── tools/
│   │   ├── scan-components.ts            # Tool: Scan the design system repo
...
│   ├── services/
│   │   ├── components-scanner.ts         # the design system repo scanner
│   │   ├── figma-matcher.ts           # Figma-to-the design system matcher
...
- `src/services/components-scanner.ts` - Scans the design system repo
...
First time setup - scan the the design system repository:
...
```typescript
// ComponentScanner scans GitHub repo
const scanner = new ComponentScanner(githubToken);
const components = await scanner.scanRepository();

// Stores in database:
{
  component_name: "Button",
  component_path: "@components/components/Button",
...
// Returns:
{
  ...
  imports: ["import { Hero, Button } from '@components/components'"],
  full_code: "<Hero>...</Hero>",
...
### `scan_repository`
Scans your design system and stores component metadata.
...
Analyzes Figma design and matches to components.
...
- `components` - Stores component metadata
...
export const analyzeFigmaTool = {
  ...
  description: 'Analyze a Figma design and match to components',
  inputSchema: {
    type: 'object',
    properties: {
      figmaUrl: {
  ...
  handler: async (args: any) => {
    ...
    // 3. Load components from database
...
### "No components found in the design system repository"
...
1. **Scan your the design system repository** to build the component database
