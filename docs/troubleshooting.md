...
- Go to SQL Editor
- Run: `SELECT * FROM components LIMIT 1;`
...
Use Button for the button
Use Panel for the card
Make it responsive
```
...
import('./dist/services/db-service.js').then(mod => {
  ...
  db.getComponents().then(r => console.log(r.length));
});
"
```
...
3. **Test with known-good:**
   - Use example repo: `mui/material-ui`
