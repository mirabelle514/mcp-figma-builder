/*
  ...
  This migration creates the core schema for mapping Figma designs to components
  in a target design system repository.

  ## New Tables

  ### `components`
  Stores metadata about available components in the target design system
  ...
  Maps Figma design patterns to components
  - `id` (uuid, primary key) - Unique identifier
  - `figma_pattern` (text) - Pattern detected in Figma (e.g., 'button', 'hero-section')
  - `component_id` (uuid) - Reference to components
...
CREATE INDEX IF NOT EXISTS idx_components_keywords ON components USING gin(figma_keywords);
CREATE INDEX IF NOT EXISTS idx_components_patterns ON components USING gin(visual_patterns);
CREATE INDEX IF NOT EXISTS idx_figma_mappings_component ON figma_component_mappings(component_id);
...
-- Policies for components (public read)
